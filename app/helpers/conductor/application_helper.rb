...
module Conductor::ApplicationHelper
  ...
  def webconsole_link
    link_to_if(defined?(WebConsole), "WebConsole", '/console') do
      link_to("WebConsole", "https://github.com/rails/web-console", target: "_blank", data: { confirm: "This extension is not installed." })
    end
  end
  ...
  def documentation_app
    if File.exist?(File.join(Rails.root, 'doc', 'app', 'index.html'))
      ...
      link_to("Your Application","#", data: { confirm: "rake doc:app generates documentation for your application in doc/app." })
    end
  end
